...
setup_factory = Setup_Stub_Factory()  # Change to Setup_Stub_Factory for testing purposes
...
detector_in_1.set_wavelength_nm(initial_wavelength_nm)
detector_in_1.set_units(detector_units)
detector_in_1.set_averaging(5)
# detector_in_1_.zero()
...
detector_in_2.set_wavelength(initial_wavelength_nm)
detector_in_2.set_units(detector_units)
detector_in_2.set_averaging(5)
# detector_in_2.zero()
...
detector_out.set_wavelength_nm(initial_wavelength_nm)
detector_out.set_units(detector_units)
detector_out.set_averaging(5)
# detector_out.zero()
...
fig.canvas.manager.window.wm_geometry('0x0+80+0')
...
pm, ws, hs = main_window(toptica_laser, detector_list)
fig.set_size_inches((ws - 80) / DPI, (hs - 200) / DPI, forward=True)

# creating the Tkinter canvas
# containing the Matplotlib figure

canvas = FigureCanvasTkAgg(fig, master=pm)
canvas.draw()
...
toolbar = NavigationToolbar2Tk(canvas, pm)
toolbar.update()
        ...
        if GUI.units_mW:
            if power_W * 1e3 > 1e-3:
                title_string += "Power " + detector.get_name() + ": " + \
                                str(np.round(power_W * 1e3, 3)) + " [mW], "
            else:
                title_string += "Power " + detector.get_name() + ": " + \
                                str(np.round(power_W * 1e6, 3)) + " [uW], "
        else:
            title_string += "Power " + detector.get_name() + ": " + \
                            str(np.round(power_W_to_dBm(power_W), 3)) + " [dBm], "
    ax.set_title(title_string)

    canvas.draw() ## Update figure
...
if GUI.sweep_flag:
    ...
    for wavelength_index in range(len(wavelength_array)):
        if GUI.sweep_flag:
            ...
            toptica_laser.set_wavelength_nm(wavelength)
