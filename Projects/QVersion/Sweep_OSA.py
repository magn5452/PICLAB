...
################################################################
# Input detectors
thorlabs_PM101U_in_1 = 'USB0::0x1313::0x8076::M00905456::INSTR'
detector_in_1 = setup_factory.create_detector(resource_manager, thorlabs_PM101U_in_1)
...
detector_in_2.set_wavelength_nm(exfo_initial_wavelength_nm)
...
## Output detector
thorlabs_PM101U_out = 'USB0::0x1313::0x8076::M00905457::INSTR'
...
## Yokogawa
yoko_conf = 'To-Be-Connected'
yoko_ = Yokogawa(resource_manager, yoko_conf)
...
if GUI.sweep_flag:
    ...
    power_W_in_2 = detector_in_2.get_detector_power()
    ...
    ##Yokogawa
    laser_wav_nm = np.zeros(len(wavelength_array))
    power_mW = np.zeros(len(wavelength_array))
    FWHM_nm = np.zeros(len(wavelength_array))

    # Fine sweep
    yoko_.set_sensitivity('MID')  # NORM/HOLD, NORM/AUTO, NORMAL, MID, HIGH1, HIGH2, HIGH3
    yoko_.set_wavelength_center((wav_max_nm - wav_min_nm) / 2)
    yoko_.set_span_wav(8)
    yoko_.set_RBW(0.05)
    wav_nm_, power = yoko_.get_trace()  # [dBm/nm]
    power_mW_nm = np.zeros([len(wavelength_array), np.shape(power)[0]])
    wav_nm = np.zeros([len(wavelength_array), np.shape(wav_nm)[0]])
    ...
    for wavelength_index in range(len(wavelength_array)):
        if GUI.sweep_flag:
            ...
            wav_nm[wavelength_index] = wav_nm_
            ind_max = np.argmax(power)
            laser_wav_nm[wavelength_index] = wav_nm_[ind_max]
            power_mW_nm = 10 ** (
                        power / 10) * 100 * 2 * 5  # 1% tap into a 50% splitter and times 5 to match photodiodes power measurement # [mW/nm]
            power_mW[wavelength_index] = np.trapz(power_mW_nm, wav_nm_)  # [mW]
            power_mW_nm[wavelength_index] = power_mW_nm
            ind_3dB = np.where(power_mW_nm >= power_mW_nm[ind_max] * 0.5)[0]
            FWHM_nm[wavelength_index] = wav_nm_[ind_3dB[-1]] - wav_nm_[ind_3dB[0]]
            ...
            print('Complete %0.2f percent\r' % (
            int((100 * (wavelength_index + 1) / len(wavelength_array)))), end='')
            plt.pause(0.1)
        else:
            break
    ...
    if GUI.save_bool:
        ...
        hf.create_dataset('laser_wav_nm', data=laser_wav_nm)
        hf.create_dataset('wav_nm', data=wav_nm)
        hf.create_dataset('power_mW_nm', data=power_mW_nm)
        hf.create_dataset('power_OSA_mW', data=power_mW)
        hf.create_dataset('FWHM_nm', data=FWHM_nm)
        hf.close()
