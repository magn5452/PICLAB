...
################################################################
from Python_lib.Lasers.Toptica_CTL950 import *
from Python_lib.Detectors.Thorlabs_PM100U import Thorlabs_PM100U
...
power_laser_mW = 40 # [mW] Laser output power 
...
camera_device_number = 1 #If no image try changing this
show_spa_sweep_plots = False # Show plots for SPA when sweeping
chip_length = 3870
image_folder = ""
...
if PHOTO_ANALYSIS:
    cam = Camera(device_number)
    cam.camsetup()
    
    image = cam.capture()
    
    spa = SPA(True,chip_length) #
...
spa = SPA(show_spa_sweep_plots,chip_length)
...
for kk in range(len(wav_array)):
    ...
    fname_base = beginning.strftime("%Y-%m-%d_%H-%M-%S") +\
                ...
                '_Run_' + \
                str(power_laser_mW) + 'mW_' + \
                str(wav_array[kk]) + 'nm_' + add_info
    image = cam.capture(image_folder + "/" + fname_base)
    
    alpha_db, r_squared, alpha_upper, alpha_lower = spa.analyze_image(image,400,100,20,10)
    alpha.append(alpha_db)
    plt.pause(0.001)
...
hf.create_dataset("alpha_dB", data=alpha)
hf.close()
